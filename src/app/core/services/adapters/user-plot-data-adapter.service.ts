...
export class UserPlotDataAdapterService implements Adapter<UserPlotData> {
  ...
  public adapt(statisticsPeriods: UserStatisticsPeriod[]): UserPlotData {
    ...
    return {
      ...
      averageSpeed: statisticsPeriods.filter(s => s.averageSpeed > 0).map(s => s.averageSpeed).length > 0 ? averageSpeed : [],
      distance: statisticsPeriods.filter(s => s.distance > 0).map(s => s.distance).length > 0 ? distance : [],
      elevation: statisticsPeriods.filter(s => s.elevation > 0).map(s => s.elevation).length > 0 ? elevation : [],
    };
  }
